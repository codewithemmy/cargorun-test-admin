...
const SideNav = () => {
  ...
  return (
    <div
      className={`${barlow.className} h-screen w-[22vw] bg-white py-10 px-2 flex flex-col items-center fixed top-0 left-0 bottom-0`}
    >
      <Image
        alt="Cargo-run Logo"
      ...
      <div className="flex flex-col space-y-6 mt-8 w-full px-6">
        {navItems.map((item) => (
          <Link
            href={item.link}
            key={item.name}
            className={`flex flex-row gap-4 w-full items-center rounded-md min-w-sm px-4 py-2 ${
              pathname === item.link && "bg-primary-blue opacity-90"
            }`}
          >
            <Image
              src={pathname === item.link ? item.white : item.icon}
              alt={item.name}
              height={20}
              width={20}
            />
