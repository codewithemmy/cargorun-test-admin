  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          const specificOrder = result.order.find(
            (item: Order) => item._id === filterId
          );
    ...
    <div className="bg-white min-h-screen mt-6 py-8 px-12 lg:pr-[10%]">
      ...
      {order ? (
        <>
              ...
              <div className="mt-10">
                ...
                <div className="mt-6 flex flex-col items-left space-y-1">
                  ...
                  <h2>
                    ...
                    {order.addressDetails.contactNumber}
                  </h2>
                </div>
              </div>
            </div>
          ...
          <div className="w-full border-b border-gray-300 my-10">
            {/* Divider */}
            <b> RIDER DETAILS </b>
            ...
            <Link href={`/riders/${order.riderId}`}>View Rider Profile</Link>
          </div>
